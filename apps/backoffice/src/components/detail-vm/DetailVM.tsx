...
export default function DetailVM() {
    ...
    const handleApplyTerraform = async () => {
        try {
            ...
            const path = `${process.env.NEXT_PUBLIC_PATH_SSH}${state.github_repo}`
            ...
            state.env = envState.envList.length === 0 && envState.envString === '' ? false : true
            ...
            <div className="flex justify-center items-center">
                <Accordion className="mt-8 rounded-t-md w-11/12 sm:w-3/4 md:w-3/4 lg:w-10/12 xl:w-10/12">
                    <Accordion.Panel className=" h-96 justify-center items-center rounded-t-md w-full">
                        <Accordion.Title onClick={(e) => e.stopPropagation()} className="bg-gray-100 h-16 px-6 rounded-t-md">Deployment Progress</Accordion.Title>
                        <div ref={outputRef} className=" px-6 py-4 h-96 overflow-y-auto overflow-x-hidden whitespace-pre-wrap">
