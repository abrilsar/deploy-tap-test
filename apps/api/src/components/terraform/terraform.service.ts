...
async function executeTerraform(reply: FastifyReply, request: FastifyRequest): Promise<void> {
  try {
    // const terraform = childProcess.spawn('terraform', ['apply', '-auto-approve'], { cwd: `${request.body?"terraform/create":`terraform/pull_request${request.body}`}` });
...
async function createScript(envState: EnvState) {
  try {
    // console.log('envList:', envState.envList);
    // const envContent = `${envState.envList
    //   .map(envVar => `${envVar.keyVar}=${envVar.valueVar}`)
    //   .join('\n')}`;
    // const content = envContent + '\n' + envState.envString
    fs.writeFileSync('./terraform/create/.env', envState.envString);
  } catch (error) {
...
async function createTerraformVar(envState: EnvState) {
  try {
    ...
    fs.writeFileSync('./terraform/create/terraform.tfvars', varContent);
  } catch (error) {
    throw new Error('500-Error creando el archivo');
  }
...
async function registerSSHKeys(envState: EnvState) {
  try {
    const response = await fetch('https://api.digitalocean.com/v2/account/keys', {
      ...
      body: JSON.stringify({
        name: envState.terraformVar.github_repo,
...
async function pullRequest(deploy: Deploy, pull: string) {
  try {
    ...
    const terraformVar = {
      ...
      pvt_key: `${process.env.PATH_SSH}${deploy.name_repo}/id_rsa`,
      pwd: process.env.PWD,
    ...
    console.log('terraformVar:', terraformVar);
...
async function deleteDroplet(deploy: Deploy) {
  try {
    const idSSH = await getId(deploy.name_repo, 'account/keys', 'ssh');
    ...
    console.log("idRecordA: ", idRecordA);
    ...
    await deleteFolder(`${process.env.PATH_SSH}${deploy.name_repo}`);
